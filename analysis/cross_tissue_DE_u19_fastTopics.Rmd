...
## GO enrichment for association with traits (MAGMA method) 

gene-set inputs: top 100 genes from each topic + immune cell markers 
GWAS: ukb-a-446
```{r}
enrich_out <- read.table("output/fastTopics/top100_topic_GO_immune.gsa.txt",
                         header = TRUE)
DT::datatable(
  enrich_out %>% mutate(P = format(P, scientific = TRUE, digits = 2))
)
```
