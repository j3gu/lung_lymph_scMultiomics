...
prop<- pdat %>% group_by(tissue, celltype, topic) %>% summarise(topic_prop=sum(proportion)/n())
...
DT::datatable(go_bp %>% 
                filter(description %in% go_diff$description & k=="k5") %>%
                mutate(FDR=format(FDR, scientific=TRUE, digits=2)) %>% 
                         select(k, description, FDR),
              caption="GO terms enriched for K5 but not K4")
```
...
DT::datatable(go_bp %>% filter(description %in% go_diff$description & k=="k6") %>%
                mutate(FDR=format(FDR, digits=2)) %>%
                select(k, description, FDR),
              caption="GO terms enriched for K6 but not K5")
...
DT::datatable(go_bp %>% filter(description %in% go_diff$description & k=="k11") %>% 
                mutate(FDR = format(FDR, digits=2)) %>%
                select(k, description, FDR),
              caption="GO terms enriched for K11 but not K10")
```
